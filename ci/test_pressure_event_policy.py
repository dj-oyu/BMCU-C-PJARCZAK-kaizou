...
class PressureEventPolicyTests(unittest.TestCase):
    def test_policy_vectors(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            executable = Path(directory) / "pressure_event_policy_test.exe"
            command = [
                zig, "c++", "-x", "c++", "-std=c++11", "-Isrc",
                "ci/pressure_event_policy_test.cpp", "-o", str(executable),
            ]
