...
class PrinterBusResultNativeTests(unittest.TestCase):
    def test_transaction_classification(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            executable = Path(directory) / "printer_bus_result_test.exe"
            command = [
                zig, "c++", "-x", "c++", "-std=c++11", "-Isrc",
                "ci/printer_bus_result_test.cpp", "-o", str(executable),
            ]
