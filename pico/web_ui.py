...
PAGE = PAGE.replace(b"</h1>", b"</h1><p><a href='/settings'>Bambuddy settings</a> / <a href='/api/pico/logs'>Pico logs JSON</a></p>", 1)
PICO_LOG_PANEL = """<h2>Pico Runtime</h2><section class="grid"><article class="card"><div class="label">Uptime</div><div class="value" id="picoUptime">--</div></article><article class="card"><div class="label">Free heap</div><div class="value" id="picoHeap">--</div></article><article class="card"><div class="label">Exceptions</div><div class="value" id="picoExceptions">--</div></article></section><details open><summary>Pico internal log (BMCU frames excluded)</summary><pre id="picoLog">No runtime log</pre></details>""".encode()
PAGE = PAGE.replace(b"<details><summary>Show diagnostic JSON</summary>", PICO_LOG_PANEL + b"<details><summary>Show diagnostic JSON</summary>", 1)
PAGE = PAGE.replace(
    b"$('raw').textContent=JSON.stringify(x,null,2)",
    b"let p=x.pico||{},logs=p.entries||[];$('picoUptime').textContent=Math.floor((p.uptime_ms||0)/1000)+' s';$('picoHeap').textContent=p.heap_free===undefined?'--':Math.floor(p.heap_free/1024)+' KiB';$('picoExceptions').textContent=val(p.exception_count,0);$('picoLog').textContent=logs.slice().reverse().map(e=>'['+e.uptime_ms+' ms] '+e.level.toUpperCase()+' '+e.component+': '+e.message).join('\\n')||'No runtime log';$('raw').textContent=JSON.stringify(x,null,2)",
    1)
