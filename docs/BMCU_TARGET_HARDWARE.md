...
convention, not by hardware protection. A build that reads `text+data =
57424` against `board_upload.maximum_size = 61440`
(`.pio/build/fw/firmware.elf`, `riscv-wch-elf-size`) has 4016 bytes of actual
headroom before the postbuild check fails, and 8112 bytes before it would
